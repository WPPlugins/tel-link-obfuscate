...
class LAOTN
{
    ...
    public static function linkToAction($args)
    {
       ...
       $out = do_shortcode('[link_obfuscate_telephone tel="' . $tel . '" link="' . $link . '" debug="' . $debug . '" link_text="' . $link_text . '" use_html_entities="' . $use_htmlentities . '" use_noscript_fallback="' . $use_noscript_fallback . '" noscript_message="' . $noscript_message . '"]');
  ...
  if(defined(LAOTN_DEBUG) || $debug == true) {
     ...
     * @static
     */
     private static function html_entities_all($text)
    {
        $debug_text = $_SESSION['debug_text'];
     ...
     * @static
     */
    private static function html_entities_closure_wrap($n, $debug_text="")
    {
        return "&#$n;";
    }
